...
export class HttpResponseHandler {
  ...
  private handleUnauthorized(responseBody: any): void {
    ...
    let unauthorizedEndpoints: Array<string> = this.configService.get(
      'notifications'
  ...
  private handleNotFound(responseBody: any): void {
    // Read configuration in order to see if we need to display 401 notification message
    let notFoundEndpoints: Array<string> = this.configService.get(
      'notifications'
    ).notFoundEndpoints;
    ...
    for (const key of Object.keys(response)) {
      if (Array.isArray(response[key])) {
        ...
        this.showNotificationError(
          'Error',
          this.getTranslatedValue(response[key])
        );
      }
    }
  }
